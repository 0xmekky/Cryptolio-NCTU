async function doAES(isEncrypt) {
    ...
    if (isEncrypt) {
        ...
        document.getElementById("text").value = btoa(String.fromCharCode(...new Uint8Array(encrypted)));
    } else {
        try {
            ...
            document.getElementById("text").value = new TextDecoder().decode(decrypted);
        } catch (err) {
